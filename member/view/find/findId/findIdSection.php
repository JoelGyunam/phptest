				...
				<table border="0" cellpadding="0" cellspacing="0" class="tbl-col-join">
					...
					<tbody>
						...
						<tr>
							...
							<td><input id="inputCode" type="text" class="input-text" style="width:302px"><a href="javascript:verifyMyCode()" class="btn-s-tin ml10">인증번호 확인</a></td>
						</tr>
					</tbody>
				</table>
			</div>
...
<script>
    ...
    function verifyMyCode(){
        ...
        $.ajax({
            ...
            ,data:{
                ...
                ,"inputCode":myCode
                ,"type":type
                ,"numberOrMail":idFinderInfo.number
            }
            ,dataType:"json"
            ,success:function(response){
                if(response.result=="success"){
                    window.location.href="/member/index.php?mode=find_id_complete";
                } else {
                    alert("잘못된 인증번호 입니다.");
                }
            }
    ...
    function getVerificationNumber(method){
        idFinderInfo = new IdFinderInfo;
        idFinderInfo.setIdFinder(method);
        if(idFinderInfo.name==""){
            alert("성명을 입력해 주세요.");
            return;
        }
        if(idFinderInfo.number=="" && idFinderInfo.method=="phone"){
    ...
    function mobileNumberMerger(){
        if($("#mobileNumberBegin").val()!="" && $("#mobileNumberCenter").val()!="" && $("#mobileNumberLast").val()!=""){
